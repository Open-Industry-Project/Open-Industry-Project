...
func _commit_preview_snap_guards(pending: Dictionary) -> void:
	...
	if not should_apply_scale and not should_open_guards:
		return
...
func _find_snap(selected: Node3D, intent: Transform3D) -> Dictionary:
	...
	var sel_features: Array = ConveyorSnapFeatures._features_of(selected)
	...
	for entry in _candidate_cache:
		...
		if sel_is_curved or entry.is_curved:
			if entry.has("curved_pairs"):
				result = _select_curved_snap(intent, entry.curved_pairs)
			else:
				result = ConveyorSnapping._calculate_curved_snap_transform(selected, tgt, true)
		elif sel_has_spur:
			...
			result = ConveyorSnapFeatures.try_snap(selected, tgt, true, sel_features, entry.features)
		if result.is_empty():
			continue
...
## Targets don't move during a drag, so cache xform/size/features/type once.
func _build_candidate_cache(selected: Node3D) -> void:
	...
	var sel_flags: Dictionary = ConveyorSnapping.live_type_cache[selected.get_instance_id()]
	var sel_end_info: Array = ConveyorSnapping._get_end_info(selected)
	for n in nodes:
		...
		var entry: Dictionary = {
			...
			"size": size,
			"features": ConveyorSnapFeatures._features_of(n),
			"is_curved": flags.is_curved,
			"has_spur": flags.has_spur,
		}
		# _snap_end_to_end only reads tgt xform + end dicts + gap, all fixed
		# during the drag — pre-compute so per-tick collapses to dist compares.
		if sel_flags.is_curved or flags.is_curved:
			entry["curved_pairs"] = _build_curved_snap_pairs(selected, n, sel_end_info, entry.xform)
		_candidate_cache.append(entry)
...
static func _select_curved_snap(intent: Transform3D, pairs: Array) -> Dictionary:
	...
	for p in pairs:
		...
		if p.flow_compatible:
			if d < best_dist:
				best_dist = d
				best = p.result
		elif d < fallback_dist:
			fallback_dist = d
			fallback = p.result
