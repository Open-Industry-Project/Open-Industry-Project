...
func _enter_tree() -> void:
	position = EditorInterface.get_editor_viewport_3d(0).get_camera_3d().position
...
func _exit_tree() -> void:
	RenderingServer.viewport_attach_camera(EditorInterface.get_editor_viewport_3d(0).get_viewport_rid(),editor_camera.get_camera_rid())
...
func handle_head_rotation():
	HEAD.rotation_degrees.y -= SimulationEvents.mouseInput.x * mouse_sensitivity
	if invert_mouse_y:
		HEAD.rotation_degrees.x -= SimulationEvents.mouseInput.y * mouse_sensitivity * -1.0
	else:
		HEAD.rotation_degrees.x -= SimulationEvents.mouseInput.y * mouse_sensitivity
	...
	SimulationEvents.mouseInput = Vector2(0,0)
...
func handle_state(moving):
	...
	if crouch_enabled:
		if crouch_mode == 0:
			if Input.is_physical_key_pressed(KEY_EQUAL) and state != "sprinting":
				if state != "crouching":
					enter_crouch_state()
