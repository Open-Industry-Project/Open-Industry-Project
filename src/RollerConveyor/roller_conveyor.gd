...
func _on_roller_added(roller: Roller) -> void:
	speed_changed.connect(roller.set_speed)
...
func _on_roller_removed(roller: Roller) -> void:
	if speed_changed.is_connected(roller.set_speed):
		speed_changed.disconnect(roller.set_speed)
...
func _on_size_changed() -> void:
	if _last_size != size:
		...
		var collision_shape_node = get_node("SimpleConveyorShape/CollisionShape3D")
		...
		var simple_conveyor_shape = get_node("SimpleConveyorShape")
		if simple_conveyor_shape:
			simple_conveyor_shape.position = Vector3(0, -0.08, 0)
