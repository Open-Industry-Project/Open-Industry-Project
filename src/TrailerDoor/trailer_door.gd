...
func _ready() -> void:
	_ensure_hole_globals_registered()
	_rebuild()
	_update_hole_params()


static func _ensure_hole_globals_registered() -> void:
	var existing := RenderingServer.global_shader_parameter_get_list()
	if not existing.has(&"trailer_hole_active"):
		RenderingServer.global_shader_parameter_add(
				&"trailer_hole_active", RenderingServer.GLOBAL_VAR_TYPE_FLOAT, 0.0)
	if not existing.has(&"trailer_hole_center"):
		RenderingServer.global_shader_parameter_add(
				&"trailer_hole_center", RenderingServer.GLOBAL_VAR_TYPE_VEC3, Vector3.ZERO)
	if not existing.has(&"trailer_hole_half_extents"):
		RenderingServer.global_shader_parameter_add(
				&"trailer_hole_half_extents", RenderingServer.GLOBAL_VAR_TYPE_VEC3, Vector3.ZERO)
