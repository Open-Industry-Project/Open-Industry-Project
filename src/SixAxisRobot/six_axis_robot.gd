...
@export_custom(0, "tag_group_enum") var tag_groups:
	set(value):
		...
		tag_groups = value
@export var j1_tag: String = ""
@export var j2_tag: String = ""
@export var j3_tag: String = ""
@export var j4_tag: String = ""
@export var j5_tag: String = ""
@export var j6_tag: String = ""
...
func _validate_property(property: Dictionary) -> void:
	...
	elif property.name in ["j1_tag", "j2_tag", "j3_tag", "j4_tag", "j5_tag", "j6_tag", "vacuum_tag"]:
		property.usage = PROPERTY_USAGE_DEFAULT if global_comms else PROPERTY_USAGE_STORAGE
...
func train_waypoint(waypoint_name: String) -> void:
	if waypoint_name.is_empty():
		...
		return
	var was_empty := waypoints.is_empty()
	waypoints[waypoint_name] = get_joint_angles()
	if was_empty:
		selected_waypoint = waypoint_name
...
func _on_simulation_started() -> void:
	...
	if not j1_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j1_tag, 1)
	if not j2_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j2_tag, 1)
	if not j3_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j3_tag, 1)
	if not j4_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j4_tag, 1)
	if not j5_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j5_tag, 1)
	if not j6_tag.is_empty():
		_register_tag_ok = _register_tag_ok and OIPComms.register_tag(tag_group_name, j6_tag, 1)
...
func _tag_group_polled(group_name: String) -> void:
	...
	if not j1_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j1_tag)
		if val != null:
			j1_angle = val
	
	if not j2_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j2_tag)
		if val != null:
			j2_angle = val
	
	if not j3_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j3_tag)
		if val != null:
			j3_angle = val
	
	if not j4_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j4_tag)
		if val != null:
			j4_angle = val
	
	if not j5_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j5_tag)
		if val != null:
			j5_angle = val
	
	if not j6_tag.is_empty():
		var val = OIPComms.read_float32(tag_group_name, j6_tag)
		if val != null:
			j6_angle = val
